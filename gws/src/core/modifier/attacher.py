...
class Attacher(BaseModifier):
	def _combine_one_point(self, other_modifier, coord, other_coord, ind):
		...
		host_atom_impl_val = self._mol_rdkit.GetAtomWithIdx(host_pos).GetImplicitValence()
		att_atom_impl_val = other_modifier._mol_rdkit.GetAtomWithIdx(att_pos).GetImplicitValence()
		for b_order in bonds_def:
			if b_order <= host_atom_impl_val and b_order <= att_atom_impl_val:
				em = Chem.EditableMol(self._mol_rdkit)
				inds_map = {}
				for at in other_modifier._mol_rdkit.GetAtoms():
	...
	def _combine_two_points(self, other_modifier, coord, other_coord, ind):
		results = []
		host_impl_val = [
			self._mol_rdkit.GetAtomWithIdx(coord[0]).GetImplicitValence(),
			self._mol_rdkit.GetAtomWithIdx(coord[1]).GetImplicitValence()]
		att_impl_val = [
			other_modifier._mol_rdkit.GetAtomWithIdx(other_coord[0]).GetImplicitValence(),
			other_modifier._mol_rdkit.GetAtomWithIdx(other_coord[1]).GetImplicitValence()]
		for b_order in bonds_def:
