...
class CoreAlgsTests(unittest.TestCase):
	...
	def test_2_unique_coords(self):
		...
		self.assertTrue(len(get_unique_coords(
			mol_graph, 1, map(lambda x: [x], xrange(mol_rdkit.GetNumAtoms())))) == 1)
		self.assertTrue(len(get_unique_coords(
		...
		mol_graph = rdkitmol2graph(mol_rdkit)
		self.assertTrue(len(get_unique_coords(
			mol_graph, 1, map(lambda x: [x], xrange(mol_rdkit.GetNumAtoms())))) == 4)
		self.assertTrue(len(get_unique_coords(
		...
		mol_graph = rdkitmol2graph(mol_rdkit)
		self.assertTrue(len(get_unique_coords(
			mol_graph, 1, map(lambda x: [x], xrange(mol_rdkit.GetNumAtoms())))) == 3)
		self.assertTrue(len(get_unique_coords(
			mol_graph, 2, map(lambda x: (x.GetBeginAtomIdx(), x.GetEndAtomIdx()), mol_rdkit.GetBonds()))) == 2)
	...
	def test_3_unique_mols(self):
		...
		for s in mols_smiles:
			mols.append(rdkitmol2graph(Chem.MolFromSmiles(s)))
